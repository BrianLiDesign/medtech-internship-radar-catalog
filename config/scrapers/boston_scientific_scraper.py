...
class BostonScientificScraper(InternshipScraper):
    """First non-Workday adapter: bostonscientific.eightfold.ai PCSX search."""
    ...
    def _search_url(self, start: int) -> str:
        ...
        return f"{CAREERS_ORIGIN}{SEARCH_PATH}?{query}"
    ...
    def _apply_url(self, position: dict) -> str:
        ...
        if path.startswith("http"):
            return path
        if path.startswith("/"):
            return f"{CAREERS_ORIGIN}{path}"
        position_id = position.get("id")
        if position_id is None:
            return ""
        return f"{CAREERS_ORIGIN}/careers/job/{position_id}"
